...
$officeTerminalSurface = Join-Path $root 'lib\office-terminal-surface.js'
...
$officeSurfaceHarness = Join-Path $root 'tests\office-terminal-surface-harness.js'
$officeClientHarness = Join-Path $root 'tests\office-client-harness.js'
$twoMachineHarness = Join-Path $root 'tests\two-machine-e2e-harness.js'
...
Assert (Test-Path -LiteralPath $officeTerminalSurface) 'The office terminal-model surface is missing.'
...
Assert (Test-Path -LiteralPath $officeSurfaceHarness) 'The office terminal-surface harness is missing.'
Assert (Test-Path -LiteralPath $officeClientHarness) 'The office single-window client harness is missing.'
Assert (Test-Path -LiteralPath $twoMachineHarness) 'The two-machine collaboration harness is missing.'
...
Assert ((Get-Content -Raw $managedRunner) -match 'createTerminalOutputCoalescer') 'The managed Codex entrypoint still paints every native redraw directly into PowerShell.'
