...
async function relayScopedAppServer({ url, threadId, sharedSessionPolicy = null, input, output, errorOutput }) {
  ...
  return new Promise((resolve, reject) => {
    function finish(error) {
      ...
      input.pause?.();
      input.destroy?.();
      // This relay belongs to an Office observer. Its normal EOF must be a
      // normal WebSocket close: terminating the socket reports 1006 to the
      // main-PC authority and can make it treat an Office disconnect as a
      // shared app-server transport failure.
      if (socket.readyState === WebSocket.OPEN) { socket.close(1000, 'Office client disconnected'); }
