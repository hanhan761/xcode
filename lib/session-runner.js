...
function createSessionGateway({ sessionId, sessionToken, processId, stateRoot, cwd, title = null, threadId = null, appServerUrl = null, getSnapshot, getDimensions, resizeTerminal, subscribeDimensions, submitRemoteMessage, subscribeOutput, isClosed }) {
  const pipeName = `\\\\.\\pipe\\xcode-session-${sessionId}`;
  const statePath = path.join(stateRoot, `${sessionId}.json`);
  fs.mkdirSync(stateRoot, { recursive: true });
  const state = {
    schemaVersion: appServerUrl ? 3 : 2,
    sessionId,
    sessionToken,
    processId,
    pipeName,
