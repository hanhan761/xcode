...
function terminalScreenText(terminal, rows) {
  const buffer = terminal.buffer.active;
  const start = Math.max(0, Math.min(buffer.viewportY, Math.max(0, buffer.length - rows)));
  const lines = [];
...
function isCodexInputBlocked(terminal, rows) {
  const screen = terminalScreenText(terminal, rows);
  return screen.includes('Do you trust the contents of this directory?') &&
    screen.includes('Press enter to continue');
}

function createSessionGateway({ sessionId, sessionToken, processId, stateRoot, cwd, title = null, threadId = null, appServerUrl = null, model = null, serviceTier, getSnapshot, getDimensions, resizeTerminal, subscribeDimensions, submitRemoteMessage, subscribeOutput, isClosed }) {
  const pipeName = `\\\\.\\pipe\\xcode-session-${sessionId}`;
  const statePath = path.join(stateRoot, `${sessionId}.json`);
  fs.mkdirSync(stateRoot, { recursive: true });
  const state = {
    schemaVersion: appServerUrl ? 4 : 2,
    sessionId,
    sessionToken,
    processId,
    pipeName,
    ...
    ...(typeof model === 'string' && model
      ? { model, serviceTier: typeof serviceTier === 'string' ? serviceTier : null }
      : {}),
  ...
  const server = net.createServer((socket) => {
    ...
    let unsubscribeDimensions = null;

    function closeConnection() {
      if (unsubscribe) { unsubscribe(); unsubscribe = null; }
      if (unsubscribeDimensions) { unsubscribeDimensions(); unsubscribeDimensions = null; }
    ...
    socket.on('data', (data) => {
      ...
      while (true) {
        ...
        if (!attached) {
          ...
          if (typeof subscribeDimensions === 'function') {
            unsubscribeDimensions = subscribeDimensions(({ cols, rows }) => {
              if (!socket.destroyed) { writeJsonLine(socket, { type: 'resized', cols, rows }); }
            });
          }
        ...
        if (frame?.type === 'resize') {
          if (!Number.isInteger(frame.cols) || !Number.isInteger(frame.rows) || frame.cols < 20 || frame.cols > 500 || frame.rows < 5 || frame.rows > 200) {
            writeJsonLine(socket, { type: 'error', code: 'INVALID_RESIZE' });
            continue;
          }
          try {
            if (typeof resizeTerminal !== 'function') { throw new Error('This session cannot resize its terminal.'); }
            resizeTerminal(frame.cols, frame.rows);
            // A subscribed session broadcasts the resolved geometry to every
            // connected office client, including this one.  Keep a direct
            // acknowledgement only for older/non-broadcast session hosts.
            if (typeof subscribeDimensions !== 'function') {
              writeJsonLine(socket, { type: 'resized', cols: frame.cols, rows: frame.rows });
            }
          }
          catch (error) {
            writeJsonLine(socket, { type: 'error', code: 'RESIZE_FAILED', message: error.message });
          }
          continue;
        }
  ...
  return {
    ...
    updateProcessId(nextProcessId) {
      if (!Number.isInteger(nextProcessId) || nextProcessId <= 0) {
        throw new RangeError('The managed Codex process id is invalid.');
      }
...
function startManagedSession({
  ...
  const terminalState = new Terminal({
    cols,
    rows,
    scrollback: 2_000,
  ...
  let terminalStateReady = false;
  let terminalStatePendingWrites = 0;
  let terminalStateWrite = Promise.resolve();
  ...
  function emitOutput(data) {
    ...
    terminalStatePendingWrites++;
    terminalStateWrite = terminalStateWrite.then(() => new Promise((resolve) => {
      terminalState.write(data, resolve);
    })).then(() => {
      terminalStatePendingWrites--;
      terminalStateReady = true;
      scheduleRemoteDrain();
    }).catch(() => {
      terminalStatePendingWrites--;
      scheduleRemoteDrain();
  ...
  function scheduleRemoteDrain() {
    clearDrainTimer();
    if (closed || remoteQueue.length === 0 || localLineOpen || !terminalStateReady || terminalStatePendingWrites > 0 || isCodexInputBlocked(terminalState, currentRows)) {
      return;
    }
...
module.exports = { createSessionGateway, normalizeRemoteMessage, startManagedSession };
