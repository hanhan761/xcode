...
function composeTerminalTitle(activityTitle, sessionTitle) {
  const activity = sanitizeSessionTitle(activityTitle);
  const session = sanitizeSessionTitle(sessionTitle);
  if (!activity) { return session || 'Codex'; }
  if (!session || activity.includes(session)) { return activity; }
  const separator = ' — ';
  const availableActivityCharacters = MAX_TITLE_CHARACTERS - Array.from(session).length - Array.from(separator).length;
  if (availableActivityCharacters < 1) { return session; }
  return `${Array.from(activity).slice(0, availableActivityCharacters).join('')}${separator}${session}`;
}

function createTerminalTitleFilter(write, initialSessionTitle = null) {
  if (typeof write !== 'function') { throw new TypeError('A terminal title filter requires a writer.'); }
  let pending = '';
  let sessionTitle = sanitizeSessionTitle(initialSessionTitle);
  let activityTitle = '';
  ...
  function drain(flush) {
    ...
    while (pending) {
      ...
      if (pending.startsWith('\x1b]0;') || pending.startsWith('\x1b]2;')) {
        ...
        activityTitle = pending.slice(4, 4 + terminator.index);
        write(terminalTitleSequence(composeTerminalTitle(activityTitle, sessionTitle)));
  ...
  return {
    ...
    setSessionTitle(nextTitle) {
      sessionTitle = sanitizeSessionTitle(nextTitle);
      write(terminalTitleSequence(composeTerminalTitle(activityTitle, sessionTitle)));
    },
    flush() { drain(true); },
  };
