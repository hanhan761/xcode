...
function readReleaseCodexPackageVersion(packageRoot) {
  const manifestPath = path.join(packageRoot, 'node_modules', '@openai', 'codex', 'package.json');
  try {
    const manifest = JSON.parse(fs.readFileSync(manifestPath, 'utf8'));
    if (typeof manifest.version === 'string' && manifest.version) { return manifest.version; }
  }
  catch {}
  throw new Error('The release-pinned Codex package manifest is missing a version. Run xcode update, then retry.');
}
...
function inspectCodexInstallation(options = {}) {
  ...
  if (source !== 'release-payload') { return { source, version }; }
  const packageVersion = readReleaseCodexPackageVersion(options.packageRoot || path.resolve(__dirname, '..'));
  if (!version.includes(packageVersion)) {
    throw new Error(`The official Codex version probe (${version}) does not match the release payload version (${packageVersion}). Run xcode update, then retry.`);
  }
  return { source, packageVersion, version };
}
...
module.exports = {
  ...
  probeCodexVersion,
  readReleaseCodexPackageVersion,
