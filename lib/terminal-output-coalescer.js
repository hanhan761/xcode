...
function createTerminalOutputCoalescer(write, {
  ...
  function render() {
    ...
    const nextLines = [];
    // DEC private mode 2026 asks compatible terminals (including current
    // Windows Terminal) to present this completed frame atomically. The Codex
    // animation stays dynamic, while the user never sees half-painted rows.
    ...
    if (changed || cursor !== renderedCursor) {
      output += `\x1b[${cursorRow};${cursorColumn}H\x1b[?7h${cursorVisible ? '\x1b[?25h' : '\x1b[?25l'}\x1b[?2026l`;
      write(output);
    }
  ...
  return {
    ...
    async close() {
      ...
      closed = true;
      terminal.dispose();
      write('\x1b[?2026h\x1b[0m\x1b[?7h\x1b[?25h\r\n\x1b[?2026l');
    },
  };
}
