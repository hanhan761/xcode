...
function waitForChildExit(child, timeoutMs = 250) {
...
async function runNativeCodexOfficeSessionAttempt({
  ...
  function relayServerMessage(text) {
    if (localSocket?.readyState === WebSocket.OPEN) { localSocket.send(text); }
    else { waitingServerMessages.push(text); }
  }

  function flushWaitingServerMessages() {
    if (!localClientReady || localSocket?.readyState !== WebSocket.OPEN) { return; }
    for (const text of waitingServerMessages.splice(0)) { localSocket.send(text); }
  }
  ...
  function handleGatewayFrame(frame) {
    if (frame.type === 'message' && typeof frame.data === 'string') {
      ...
      try {
        ...
        if (resumeRequestId !== null && !Object.hasOwn(message, 'method') && Object.hasOwn(message, 'id') && message.id === resumeRequestId) {
          relayServerMessage(text);
          if (!Object.hasOwn(message, 'error')) {
            localClientReady = true;
            resumeRequestId = null;
            flushWaitingServerMessages();
          }
          return;
        }
        if (typeof message.method === 'string' && !localClientReady) {
          waitingServerMessages.push(text);
          return;
        }
      }
  ...
  server.on('connection', (socket) => {
    ...
    socket.on('message', (data, isBinary) => {
      ...
      try {
        const message = JSON.parse(text);
        if (message.method === 'thread/resume' && message.params?.threadId === session.threadId) {
          localClientReady = false;
          resumeRequestId = message.id;
        }
      }
      catch { /* The main-PC gateway validates the official Codex client message. */ }
      try { sendBridgeFrame(text); }
  ...
  async function cleanup() {
    ...
    if (bridge) {
      bridge.stdin.end();
      // Let the forced gateway relay its normal WebSocket close to the main
      // authority before terminating it. A forced kill turns an Office tab
      // exit into an abnormal remote transport reset for the main session.
      const bridgeExited = await waitForChildExit(bridge);
      if (!bridgeExited && !bridge.killed) { bridge.kill(); }
    }
  }
