...
async function runNativeCodexOfficeSession({
  ...
  try {
    ...
    codex = spawnProcess(codexExecutable, ['resume', '--remote', localUrl, '--no-alt-screen', session.threadId], {
      cwd,
      env,
      stdio: 'inherit',
      windowsHide: true,
    });
    const codexResult = new Promise((resolve, reject) => {
      codex.once('error', reject);
      codex.once('exit', (code, signal) => resolve({ code: code ?? 1, signal }));
    });
    const result = await Promise.race([codexResult, runtimeFailure]);
    return result.code;
  }
  finally {
    if (codex && !codex.killed && codex.exitCode === null) { codex.kill(); }
    await cleanup();
  }
}
