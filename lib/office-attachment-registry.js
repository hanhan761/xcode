...
function createOfficeAttachmentRegistry({
  ...
  function claim({ threadId, sessionId, attachmentToken, processId }) {
    const ownerProcessId = processId ?? process.pid;
    if (!Number.isInteger(ownerProcessId) || ownerProcessId <= 0) { throw new TypeError('An office attachment claim requires a live process id.'); }
    ensureRoot();
    ...
    const claimRecord = { ...reservation, processId: ownerProcessId, pending: false, createdAt: now() };
    write(claimRecord);
    return claimRecord;
  }
