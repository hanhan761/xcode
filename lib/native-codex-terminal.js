...
const { createTerminalTitleFilter } = require('./session-title');
...
function startNativeCodexTerminal({
  ...
  const terminalOutput = createTerminalTitleFilter((data) => sink.write(data), initialTitle);
  let exited = false;
  let stopped = false;
  ...
  if (initialTitle) { terminalOutput.setSessionTitle(initialTitle); }
  ...
  return {
    ...
    completed,
    setTitle(nextTitle) {
      terminalOutput.setSessionTitle(nextTitle);
    },
    stop,
  };
}
