...
async function acquireDedicatedAppServer({ file, cwd, env }) {
  ...
  const child = spawn(file, ['--disable', 'computer_use', 'app-server', '--listen', url], { cwd, env: hiddenShell.env, stdio: ['ignore', 'pipe', 'pipe'], windowsHide: true });
  child.stdout.on('data', (data) => logs.push(data.toString()));
  child.stderr.on('data', (data) => logs.push(data.toString()));
  try {
    await waitForAppServer(url);
  }
  catch (error) {
