...
async function resolveThreadRequest(authority, args, cwd) {
  if (args[0] === 'resume' && args[1] === '--last') {
    ...
    return { method: 'thread/resume', params: { threadId: latest.id, cwd } };
  }
  if (args[0] === 'resume' && /^[0-9a-f]{8}-[0-9a-f-]{27}$/i.test(args[1] || '')) {
    return { method: 'thread/resume', params: { threadId: args[1], cwd } };
  }
  const params = { cwd };
...
module.exports = { AppServerClient, startSharedAppServerSession };
