...
const { createAuthoritativeWorkingReconciler, hasVisibleNativeWorkingSpinner } = require('./authoritative-working-state');
...
function applyCodexOverrides(params, args) {
  for (let index = 0; index < args.length; index += 1) {
    if ((args[index] === '--model' || args[index] === '-m') && args[index + 1]) {
      ...
      const configOverride = args[index] === '--config' || args[index] === '-c'
        ? args[index + 1]
        : args[index].startsWith('--config=') ? args[index].slice('--config='.length) : null;
      const serviceTier = typeof configOverride === 'string'
        ? /^service_tier\s*=\s*["']?([^"']+?)["']?\s*$/.exec(configOverride)?.[1]
        : null;
      if (serviceTier) {
...
function parseResumeInvocation(args) {
  if (!Array.isArray(args) || args[0] !== 'resume') { return null; }
  const overrides = {};
  applyCodexOverrides(overrides, args.slice(1));
  const params = {};
  if (overrides.model) { params.model = overrides.model; }
  if (overrides.serviceTier) { params.serviceTier = overrides.serviceTier; }
  return {
    useLast: args.includes('--last'),
    threadId: args.slice(1).find((arg) => /^[0-9a-f]{8}-[0-9a-f-]{27}$/i.test(arg || '')) || null,
    params,
  };
}

async function resolveThreadRequest(authority, args, cwd) {
  const resume = parseResumeInvocation(args);
  if (resume?.useLast) {
    ...
    return { method: 'thread/resume', params: { threadId: latest.id, ...resume.params } };
  }
  if (resume?.threadId) {
    return { method: 'thread/resume', params: { threadId: resume.threadId, ...resume.params } };
  }
  const params = { cwd };
  applyCodexOverrides(params, args);
...
async function initializeNewThreadForRemoteTui(authority, threadId, {
  ...
  // rejects it with "no rollout found". An accepted empty turn creates that
  // internal rollout without injecting a user message into the conversation.
  // Do not attach the native TUI while this invisible turn is active: Codex
  // renders it as a user-visible Working state.  Once the server has told us
  // it is active, interrupt it and wait for its authoritative completion.
  // This leaves a resumable rollout without exposing bootstrap work as a turn.
  const startedTurnIds = new Set();
  const completedTurnIds = new Set();
  let startedWithoutId = false;
  let completedWithoutId = false;
  const unsubscribe = authority.onNotification((event) => {
    ...
    if (event.method === 'turn/started') {
      if (turnId) { startedTurnIds.add(turnId); }
      else { startedWithoutId = true; }
    }
    if (event.method === 'turn/completed') {
      if (turnId) { completedTurnIds.add(turnId); }
      else { completedWithoutId = true; }
    }
  });
  try {
    const accepted = await requestWithTimeout(
      () => authority.request('turn/start', { threadId, input: [] }),
      requestTimeoutMs,
      'the initial Codex bootstrap request',
    );
    const bootstrapTurnId = accepted.turn?.id || accepted.turnId;
    if (!bootstrapTurnId) {
      throw new Error('The initial Codex bootstrap request did not return a turn id.');
    }
    const isStarted = () => startedWithoutId || startedTurnIds.has(bootstrapTurnId);
    const isCompleted = () => completedWithoutId || completedTurnIds.has(bootstrapTurnId);
    await waitFor(
      () => isStarted() || isCompleted(),
      lifecycleTimeoutMs,
      'the initial Codex bootstrap turn to become active',
    );
    if (!isCompleted()) {
      try {
        await requestWithTimeout(
          () => authority.request('turn/interrupt', { threadId, turnId: bootstrapTurnId }),
          requestTimeoutMs,
          'the initial Codex bootstrap turn interruption',
        );
      }
      catch {
        // A completion can win the race with the interruption request. Its
        // notification below remains the authority for when the TUI is safe.
      }
    }
    await waitFor(
      isCompleted,
      lifecycleTimeoutMs,
      'the initial Codex bootstrap turn to complete',
    );
...
async function startSharedAppServerSession({
  ...
  let unsubscribeWorkingState = () => {};
  let scrollback = '';
  let currentCols = cols;
  let currentRows = rows;
  const tuiScreen = new Terminal({
    cols,
    rows,
    scrollback: 0,
  ...
  let tuiScreenWrite = Promise.resolve();
  let workingReconciler = null;
  let restartingTui = false;
  ...
  function emitOutput(data) {
    ...
    tuiScreenWrite = tuiScreenWrite
      .then(() => new Promise((resolve) => tuiScreen.write(data, resolve)))
      .catch(() => {});
    for (const listener of outputListeners) { listener(data); }
  }

  async function visibleTuiScreen() {
    await tuiScreenWrite;
    const buffer = tuiScreen.buffer.active;
    return Array.from({ length: currentRows }, (_, index) =>
      buffer.getLine(buffer.viewportY + index)?.translateToString(true) || '').join('\n');
  }

  function resetTuiScreen() {
    tuiScreenWrite = tuiScreenWrite.then(() => { tuiScreen.reset(); }).catch(() => {});
  }
  ...
  try {
    ...
    currentModel = typeof started.model === 'string' && started.model ? started.model : null;
    currentServiceTier = typeof started.serviceTier === 'string' && started.serviceTier ? started.serviceTier : null;
    ...
    function startNativeTui() {
      const nextTui = pty.spawn(file, ['resume', '--remote', host.url, '--no-alt-screen', threadId], {
        name: 'xterm-256color', cols: currentCols, rows: currentRows, cwd, env, useConptyDll: true,
      });
      tui = nextTui;
      gateway?.updateProcessId(nextTui.pid);
      resetTuiScreen();
      nextTui.onData((data) => {
        if (tui === nextTui) { emitOutput(data); }
      });
      nextTui.onExit(({ exitCode, signal }) => {
        if (tui !== nextTui) { return; }
        cleanup().catch(() => {}).finally(() => resolveCompleted({ exitCode, signal }));
      });
    }

    async function restartNativeTui() {
      if (closed || restartingTui || !tui) { return; }
      restartingTui = true;
      const previousTui = tui;
      try {
        startNativeTui();
        previousTui.kill();
      }
      finally { restartingTui = false; }
    }

    startNativeTui();
    workingReconciler = createAuthoritativeWorkingReconciler({
      threadId,
      readThreadStatus: async () => {
        const read = await authority.request('thread/read', { threadId, includeTurns: false });
        return read.thread?.status?.type || read.status?.type || null;
      },
      isWorkingSpinnerVisible: async () => hasVisibleNativeWorkingSpinner(await visibleTuiScreen()),
      restartTui: restartNativeTui,
    });
    unsubscribeWorkingState = authority.onNotification((event) => workingReconciler.onNotification(event));

    gateway = createSessionGateway({
      sessionId,
      sessionToken: crypto.randomBytes(32).toString('base64url'),
      get processId() { return tui.pid; },
      stateRoot,
      cwd: sessionCwd,
      title: currentTitle,
      threadId,
      ...
      resizeTerminal(nextCols, nextRows) {
        ...
        tuiScreen.resize(nextCols, nextRows);
    ...
    return {
      ...
      get serviceTier() { return currentServiceTier; },
      get processId() { return tui.pid; },
...
module.exports = { AppServerClient, initializeNewThreadForRemoteTui, parseResumeInvocation, resolveThreadRequest, shouldShareAppServer, startSharedAppServerSession };
