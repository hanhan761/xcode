...
async function startSharedAppServerSession({
  ...
  try {
    ...
    // Native Codex normally uses the alternate screen, which deliberately has
    // no terminal scrollback. Keep the managed native TUI in Codex's own
    // no-alt-screen mode so the paired office mirror can review prior output
    // without sending navigation keys into the main PC's keyboard.
    tui = pty.spawn(file, ['resume', '--remote', url, '--no-alt-screen', threadId], {
