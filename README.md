...
办公本只显示主力机**当前仍在运行**的受控 Codex 会话；已退出、崩溃或仅保存在 Codex 历史中的会话不会出现。若有多个活会话，选择一个。办公本会先收到当前终端快照与后续输出；在办公本键入一条消息并回车后，xcode 会显示“已排队”，并在真正写入主力机终端后显示“已送达”。`已排队` 只会在主力机确实保留了未提交的本地草稿时短暂停留；方向键、历史导航、退格清空和 Ctrl+C 不会再阻塞远端。主力机仍可同时观察和继续工作。
