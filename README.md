...
办公本只显示主力机**当前仍在运行**的受控 Codex 会话；已退出、崩溃或仅保存在 Codex 历史中的会话不会出现。若有多个活会话，选择一个。办公本会先收到当前终端快照与后续输出；在办公本键入一条消息并回车后，xcode 会显示“已排队”，并在真正写入主力机终端后显示“已送达”。主力机仍可同时观察和继续工作。
