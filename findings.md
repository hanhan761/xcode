...
- The source-level readiness and native-Codex probes are not by themselves sufficient release evidence: the final check must exercise the files actually produced by `npm pack` and installed into a clean prefix, without mutating the globally installed package or interrupting active sessions.
- The exact `xcode-remote-1.3.0.tgz` artifact was installed into an isolated temporary npm prefix and passed all three behavioral checks: safety-prompt queueing, the complete forced-gateway/two-machine route, and a real native-Codex challenge/acknowledgement. The original native probe's fixed three-second Enter was a test race, not a production-package difference; waiting for the visible prompt makes the test deterministic. No probe processes remained after the successful run.
- Live diagnosis on 2026-07-17: the installed main-PC package now contains the readiness gate and its forced gateway lists five reachable managed sessions, including `D:\\ROGII`. A read-only snapshot of the ROGII session shows `Goal achieved (6h 55m)` as the final visible Codex state, not a normal active composer. The current catalog treats process + private-pipe reachability as sufficient liveness, so it can advertise a completed-but-not-exited TUI as an interactive conversation. This is a semantic-liveness defect that exactly matches “remote write acknowledged, neither side starts work”.
- Follow-up read-only transcript inspection changes the ranking: the ROGII scrollback has two visible `收割一下` turns, each followed by a substantive Codex response; the later one says the Kaggle private-v2 result was checked/harvested and closed. Thus the office-originated text can reach Codex and Codex can act on it. The remaining live symptom is output observation/session selection or a disconnected office client, not terminal-input delivery. The `Goal achieved` footer must not be treated as conclusive non-interactivity without a focused regression because it appears after legitimate completed turns.
- Root cause for the live symptom: Windows OpenSSH Operational events show the office device `100.86.255.12` started its forced gateway attachment at 11:41:55 and the server logged `Read error from remote host ... Connection timed out` at 11:45:57. There was no established TCP/22 connection afterward. The persistent office `xcode` attach has no SSH application keepalive, so an idle four-minute period destroys the only live mirror/input channel. Add `ServerAliveInterval`, `ServerAliveCountMax` and TCP keepalive options on the office client attachment.
- Repair: only `xcode`'s long-lived SSH `attach` process now uses `ServerAliveInterval=30`, `ServerAliveCountMax=3`, and `TCPKeepAlive=yes`; short `list` calls remain unchanged. The new office-client regression is red against the prior client and asserts all three SSH arguments. It passed both from the source checkout and from a freshly packed-and-isolated npm installation; the full forced gateway/two-machine test also passed against that installed artifact.
- Architecture correction after the user reported a fork: `bin/managed-codex.js` calls `startManagedSession`, which always spawns a new native `codex.exe` and has no Codex thread identifier. Its tests prove terminal-byte delivery to that child, not delivery to an arbitrary pre-existing main Codex conversation. The official app-server API is the correct semantic seam: it exposes `thread/resume`, `thread/read`, `turn/start`, and thread turn/item event subscriptions. The current PTY transport cannot honestly promise one conversation identity across the main surface and office laptop.

## One-thread implementation verification

- The replacement authority is a loopback-only native `codex app-server`. `managed-codex.js` starts a thread through that server, then starts the normal native Codex TUI with `codex resume --remote ws://127.0.0.1:PORT <threadId>`. It no longer starts a separate conversation-bearing Codex child.
- The forced SSH gateway remains the only cross-device path. An office message becomes `turn/start` for the already-authorized `threadId`; it is not converted to pseudo-terminal keystrokes. The main TUI receives the same app-server thread event and renders the same response.
- `APP_SERVER_SHARED_THREAD=PASS` used two independent WebSocket clients plus the native remote TUI: the office-side client resumed the main thread, started a uniquely marked turn, and both the main app-server subscriber and main TUI observed its response.
- `SHARED_SESSION_RUNNER=PASS` exercised the actual product runner: create loopback authority, start the main native TUI, submit an office message, and observe its exact response in the main TUI stream. The temporary test thread was deleted afterwards.
- The only intentional migration boundary is product surfaces that cannot provide a local CLI thread id (for example, an arbitrary desktop task). They cannot be silently merged. A normal local CLI thread is migrated by `codex resume --last` or `codex resume <threadId>` in the project directory.
